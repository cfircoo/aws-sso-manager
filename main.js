...
console.log(
  "App path:",
  isElectronApp ? electronApp.getAppPath() : process.cwd()
);
...
const getSettings = () => {
  try {
    if (fs.existsSync(settingsFilePath)) {
      ...
      if (settings.session && settings.session.expirationTime) {
        const now = Date.now();
        const isExpired = settings.session.expirationTime <= now;
        console.log(
          `[Settings] Session loaded: ${isExpired ? "EXPIRED" : "VALID"}`,
          `Expires: ${new Date(
            settings.session.expirationTime
          ).toLocaleString()}`
        );

        // If session is expired, clear it immediately
        if (isExpired) {
          console.log("[Settings] Clearing expired session");
...
const saveSession = (accessToken, expirationTime) => {
  try {
    ...
    saveSettings(settings);
    console.log(
      "Session saved, expires:",
      new Date(settings.session.expirationTime).toLocaleString()
    );
    return true;
  } catch (error) {
...
const getSession = () => {
  try {
    // Always get fresh settings to avoid stale session data
    const settings = getSettings();
    ...
    if (
      ...
      console.log("No saved session found in getSession");
      return null;
    }

    // Check validity with current time
    const now = Date.now();
    const isValid = settings.session.expirationTime > now;

    if (!isValid) {
      console.log(
        "Session expired in getSession:",
        "Expires:",
        new Date(settings.session.expirationTime).toLocaleString(),
        "Current:",
...
const setDefaultProfile = (accountId, roleName) => {
  try {
    ...
    saveSettings(settings);
    console.log(
      "Default profile set in app settings:",
      settings.defaultProfile
    );
...
`;
    ...
    if (!configContent.trim()) {
      ...
      if (configContent.includes(defaultProfileMarker)) {
        ...
        if (
          inDefaultProfile &&
          !profileLines[profileLines.length - 1].trim().startsWith("[")
        ) {
...
function createWindow() {
  ...
  try {
    ...
    // Show window when ready to load to prevent visual issues
...
# Terminal launched for role ${env.AWS_ROLE_NAME || "unknown"} in account ${
...
`;
    ...
    if (env.AWS_ACCESS_KEY_ID && env.AWS_SECRET_ACCESS_KEY) {
      ...
      const expiration = env.AWS_EXPIRATION
        ? parseInt(env.AWS_EXPIRATION, 10)
        : 0;
...
`;
    ...
    const escapedEnvFilePath = envFilePath
      .replace(/"/g, '\\"')
      .replace(/'/g, "\\'")
      .replace(/ /g, "\\ ");
...
`
    ...
    return {
      success: false,
      error: error.message || "Failed to open macOS terminal",
    };
  }
}
...
ipcMain.handle("aws-sso:open-terminal", async (_, options) => {
  ...
  try {
    ...
    if (
      ...
      awsWhoAmIFunction +=
        '  echo "--- AWS Identity Info (from aws_who_am_i) ---";\n';
      // Escape $ for shell, accountId/roleName are already shell-escaped JS variables
      awsWhoAmIFunction +=
        '  echo "AWS Account: \\${AWS_PROMPT_ACCOUNT:-Not Set} (' +
        accountId +
        ')";\n';
      awsWhoAmIFunction +=
        '  echo "AWS Role:    \\${AWS_PROMPT_ROLE:-Not Set} (' +
        roleName +
        ')";\n';
      ...
      awsWhoAmIFunction += '  echo "";\n';
      awsWhoAmIFunction +=
        '  echo "Current AWS Environment Variables (from env):";\n';
      awsWhoAmIFunction +=
        '  env | grep AWS || echo "No AWS variables found in env";\n';
      awsWhoAmIFunction +=
        '  echo "---------------------------------------------";\n';
      awsWhoAmIFunction += "}\n"; // Closing brace
...
`;
    ...
    console.log(
      "[Main] Generated Script Content (first 500 chars):\n",
    ...
    try {
      ...
      console.error("Failed to write error to debug log:", logError);
    }
    return {
      success: false,
...
async function openWindowsTerminal(env, debugLogPath) {
  ...
  try {
    ...
    if (env.AWS_ACCESS_KEY_ID && env.AWS_SECRET_ACCESS_KEY) {
      ...
      const expiration = env.AWS_EXPIRATION
        ? new Date(parseInt(env.AWS_EXPIRATION, 10)).toLocaleString()
        : "N/A";
      ...
      batchContent += "\r\n";
      batchContent +=
        'echo Type "call :aws_info" to see AWS identity information again\r\n';
      batchContent += "echo.\r\n";
    }
    ...
    return {
      success: false,
      error: error.message || "Failed to open Windows terminal",
    };
  }
}
...
if (isElectronApp && ipcMain) {
  ...
  ipcMain.handle("aws-sso:poll-token", async (_, { deviceCode }) => {
    try {
      if (!ssoOidcClient || !clientId || !clientSecret) {
        throw new Error(
          "SSO client not initialized or missing client credentials"
        );
      }
  ...
  ipcMain.handle(
    ...
    async (_, { accessToken, accountId, roleName }) => {
      try {
        if (!ssoClient) throw new Error("SSO client not initialized");
        console.log(
          `Getting credentials for account ${accountId} and role ${roleName}`
        );
  ...
  ipcMain.handle(
    "aws-sso:set-default-profile",
    async (_, { accountId, roleName }) => {
  ...
  ipcMain.handle("aws-sso:login-ecr", async (_, { accountId, roleName }) => {
    ...
    try {
      ...
      const awsPath = await findAwsCli();
      console.log(`[Main] Using AWS CLI at: ${awsPath}`);
      ...
      const { stdout: password, stderr: passError } = await execAsync(
        `${awsPath} ecr get-login-password --region ${currentRegion}`,
        {
          env: {
            ...process.env,
  ...
  ipcMain.handle(
    "aws-sso:login-codeartifact",
    async (_, { accountId, roleName }) => {
      console.log("[Main] CodeArtifact login requested:", {
        accountId,
        roleName,
      });
      ...
      try {
        ...
        if (checkResult.includes("❌")) {
          ...
          const { stderr: loginError } = await execAsync(
            `${awsPath} codeartifact login --tool pip --domain your-domain --domain-owner ${accountId} --region ${currentRegion}`,
            { env: credentialsEnv }
          );
  ...
  // Add EKS cluster listing handler
  ipcMain.handle(
    "aws-sso:list-eks-clusters",
    async (_, { accessToken, accountId, roleName, region }) => {
      console.log("[Main] EKS clusters listing requested:", {
        accountId,
        roleName,
        region,
      });
      ...
      try {
        ...
        // Get credentials for the role
        console.log("[Main] Getting role credentials for EKS cluster listing");
        const command = new GetRoleCredentialsCommand({
          accessToken,
          accountId,
          roleName,
        ...
        // Create EKS client with the role credentials
        ...
        // List EKS clusters
        console.log("[Main] Listing EKS clusters");
        ...
        console.log(`[Main] Found ${clusters.length} EKS clusters`);
        ...
        const clusterDetails = await Promise.all(
          clusters.map(async (clusterName) => {
            try {
              ...
              const clusterDetail = await eksClient.send(describeCommand);
              return {
                name: clusterName,
                status: clusterDetail.cluster?.status,
                version: clusterDetail.cluster?.version,
                endpoint: clusterDetail.cluster?.endpoint,
                arn: clusterDetail.cluster?.arn,
                region: targetRegion,
              };
            } catch (error) {
              console.error(
                `[Main] Error getting details for cluster ${clusterName}:`,
                error
              );
              return {
                name: clusterName,
        ...
        console.error("[Main] EKS cluster listing failed:", error);
        return {
          success: false,
          message: error.message || "Failed to list EKS clusters",
          clusters: [],
          timestamp: Date.now(),
        };
      }
  ...
  // Add kubectl context switching handler
  ipcMain.handle(
    "aws-sso:set-kubectl-context",
    async (_, { accessToken, accountId, roleName, clusterName, region }) => {
      console.log("[Main] Kubectl context switch requested:", {
        accountId,
        roleName,
        clusterName,
        region,
      ...
      try {
        // Skip the old kubectl check - we'll use bulletproof detection below
        ...
        // Get credentials for the role
        console.log(
          "[Main] Getting role credentials for kubectl context setup"
        );
        const command = new GetRoleCredentialsCommand({
          accessToken,
          accountId,
        ...
        // Find AWS CLI executable using bulletproof detection
        const awsPath = await detectAwsCli("context-setting");

        // Update AWS kubeconfig using aws eks update-kubeconfig
        console.log("[Main] Updating kubeconfig for EKS cluster");
        const updateKubeconfigCmd = `${awsPath} eks update-kubeconfig --region ${targetRegion} --name ${clusterName} --alias ${accountId}-${clusterName}`;

        const { stdout: kubeconfigOutput, stderr: kubeconfigError } =
          await execAsync(updateKubeconfigCmd, {
            env: {
              ...process.env,
              AWS_ACCESS_KEY_ID: response.roleCredentials.accessKeyId,
              AWS_SECRET_ACCESS_KEY: response.roleCredentials.secretAccessKey,
        ...
        if (kubeconfigError && !kubeconfigOutput) {
          throw new Error(`Failed to update kubeconfig: ${kubeconfigError}`);
        }

        // Find kubectl executable using bulletproof detection
        const kubectlPath = await detectKubectl("context-setting");

        // Set the kubectl context
        const contextName = `${accountId}-${clusterName}`;
        console.log("[Main] Setting kubectl context to:", contextName);
        const setContextCmd = `${kubectlPath} config use-context ${contextName}`;

        const { stdout: contextOutput, stderr: contextError } = await execAsync(
          setContextCmd
        );

        if (contextError && !contextOutput) {
          throw new Error(`Failed to set kubectl context: ${contextError}`);
        }

        // Verify the context is set
        console.log("[Main] Verifying kubectl context");
        const { stdout: currentContext } = await execAsync(
          `${kubectlPath} config current-context`
        );

        console.log("[Main] Kubectl context set successfully");
        return {
          success: true,
          message: `Successfully set kubectl context to ${clusterName}`,
        ...
        console.error("[Main] Kubectl context switch failed:", error);
        ...
        return {
          success: false,
          message: error.message || "Failed to set kubectl context",
          timestamp: Date.now(),
        };
      }
    }
  ...
  // Global storage for verified tool paths
  let verifiedKubectlPath = null;
  let verifiedAwsPath = null;
  let kubectlVerificationTime = null;
  let awsVerificationTime = null;

  // Helper function to clear tool caches (useful for debugging)
  const clearToolCaches = () => {
    console.log("[Main] Clearing all tool path caches and verified paths");
    verifiedKubectlPath = null;
    verifiedAwsPath = null;
    kubectlVerificationTime = null;
    awsVerificationTime = null;
  };

  // Simplified and bulletproof kubectl detection
  const detectKubectl = async (operation = "general") => {
    console.log(`[Main] ===== KUBECTL DETECTION (${operation}) =====`);

    const fs = require("fs");

    // Define all possible kubectl locations
    const kubectlPaths = [
      "/usr/local/bin/kubectl", // Standard Homebrew location
      "/opt/homebrew/bin/kubectl", // M1 Mac Homebrew location
      "/usr/bin/kubectl", // System location
      "/snap/bin/kubectl", // Snap package location
      process.env.HOME + "/bin/kubectl",
      process.env.HOME + "/.local/bin/kubectl",
      "/Applications/Docker.app/Contents/Resources/bin/kubectl",
    ];

    // Add Homebrew versioned paths dynamically
    const brewPrefixes = ["/usr/local", "/opt/homebrew"];
    for (const prefix of brewPrefixes) {
      try {
        const cellarPath = `${prefix}/Cellar/kubernetes-cli`;
        if (fs.existsSync(cellarPath)) {
          const versions = fs.readdirSync(cellarPath);
          for (const version of versions) {
            kubectlPaths.push(`${cellarPath}/${version}/bin/kubectl`);
          }
        }
      } catch (error) {
        // Ignore errors reading Homebrew directories
      }
    }

    console.log(`[Main] Testing ${kubectlPaths.length} kubectl locations...`);

    // Test each path directly with detailed logging
    for (const kubectlPath of kubectlPaths) {
      console.log(`[Main] Checking: ${kubectlPath}`);
      ...
      try {
        ...
        const stats = fs.statSync(kubectlPath);
        if (!stats.isFile()) {
          console.log(`[Main]   → Not a file (is directory or other)`);
          continue;
        }

        console.log(`[Main]   → File exists, testing execution...`);

        // Test if kubectl actually works
        ...
        if (
          stdout &&
          (stdout.includes("Client Version") || stdout.includes("v1."))
        ) {
          console.log(`[Main] ✅ SUCCESS: kubectl found at ${kubectlPath}`);
          console.log(`[Main]   → Version: ${stdout.trim()}`);

          // Cache the successful path
          verifiedKubectlPath = kubectlPath;
          kubectlVerificationTime = Date.now();

          return kubectlPath;
        } else {
          console.log(`[Main]   → Invalid response: ${stdout}`);
        }
      } catch (error) {
    ...
    // Fallback 1: Try PATH-based detection
    console.log("[Main] Fallback 1: PATH-based detection");
    try {
      const { stdout } = await execAsync("which kubectl");
      const kubectlPath = stdout.trim();
      if (kubectlPath) {
        console.log(`[Main] Found kubectl via PATH: ${kubectlPath}`);
        const { stdout: versionOutput } = await execAsync(
          `"${kubectlPath}" version --client`
        );
        if (
          versionOutput &&
          (versionOutput.includes("Client Version") ||
            versionOutput.includes("v1."))
        ) {
          console.log(`[Main] ✅ SUCCESS: kubectl verified via PATH`);
          verifiedKubectlPath = kubectlPath;
          kubectlVerificationTime = Date.now();
          return kubectlPath;
        }
      ...
      console.log(`[Main] PATH detection failed: ${error.message}`);
    }

    // Fallback 2: Shell environment with full profile loading
    console.log("[Main] Fallback 2: Shell environment detection");
    try {
      const { stdout } = await execAsync(
        "bash -l -c 'which kubectl 2>/dev/null || echo NOTFOUND'"
      );
      const kubectlPath = stdout.trim();
      if (kubectlPath && kubectlPath !== "NOTFOUND") {
        console.log(`[Main] Found kubectl via shell: ${kubectlPath}`);
        const { stdout: versionOutput } = await execAsync(
          `"${kubectlPath}" version --client`
        );
        if (
          ...
          console.log(`[Main] ✅ SUCCESS: kubectl verified via shell`);
          verifiedKubectlPath = kubectlPath;
          kubectlVerificationTime = Date.now();
          return kubectlPath;
        }
      ...
      console.log(`[Main] Shell detection failed: ${error.message}`);
    }

    // If we get here, kubectl was not found anywhere
    console.error(`[Main] ===== KUBECTL NOT FOUND (${operation}) =====`);
    console.error("[Main] Searched paths:");
    for (const path of kubectlPaths) {
      console.error(`[Main]   - ${path}`);
    }
    console.error("[Main] Environment info:", {
      HOME: process.env.HOME,
      PATH: process.env.PATH
        ? process.env.PATH.split(":").slice(0, 5).join(":") + "..."
        : "undefined",
      isPackaged: electronApp ? electronApp.isPackaged : "unknown",
      platform: process.platform,
    });

    throw new Error(
      "kubectl is not installed or not found. Please install kubectl using 'brew install kubectl' or download from https://kubernetes.io/docs/tasks/tools/install-kubectl-macos/"
    );
  };

  // Simplified and bulletproof AWS CLI detection
  const detectAwsCli = async (operation = "general") => {
    console.log(`[Main] ===== AWS CLI DETECTION (${operation}) =====`);

    const fs = require("fs");

    // Define all possible AWS CLI locations
    const awsPaths = [
      "/usr/local/bin/aws", // Standard Homebrew location
      "/opt/homebrew/bin/aws", // M1 Mac Homebrew location
      "/usr/bin/aws", // System location
      "/usr/local/aws-cli/v2/current/bin/aws", // AWS installer location
      process.env.HOME + "/bin/aws",
      process.env.HOME + "/.local/bin/aws",
      process.env.HOME + "/.pyenv/shims/aws",
    ];

    console.log(`[Main] Testing ${awsPaths.length} AWS CLI locations...`);

    // Test each path directly with detailed logging
    for (const awsPath of awsPaths) {
      console.log(`[Main] Checking: ${awsPath}`);
      ...
      try {
        ...
        // Check if it's a file (not directory)
        const stats = fs.statSync(awsPath);
        if (!stats.isFile()) {
          console.log(`[Main]   → Not a file (is directory or other)`);
          continue;
        }
        ...
        // Test if AWS CLI actually works
        const { stdout, stderr } = await execAsync(`"${awsPath}" --version`);

        if (stdout && (stdout.includes("aws-cli") || stdout.includes("aws/"))) {
          console.log(`[Main] ✅ SUCCESS: AWS CLI found at ${awsPath}`);
          console.log(`[Main]   → Version: ${stdout.trim()}`);

          // Cache the successful path
          verifiedAwsPath = awsPath;
          awsVerificationTime = Date.now();
    ...
    // Fallback 1: Try PATH-based detection
    console.log("[Main] Fallback 1: AWS CLI PATH-based detection");
    try {
      const { stdout } = await execAsync("which aws");
      const awsPath = stdout.trim();
      if (awsPath) {
        console.log(`[Main] Found AWS CLI via PATH: ${awsPath}`);
        const { stdout: versionOutput } = await execAsync(
          `"${awsPath}" --version`
        );
        if (
          versionOutput &&
          (versionOutput.includes("aws-cli") || versionOutput.includes("aws/"))
        ) {
          console.log(`[Main] ✅ SUCCESS: AWS CLI verified via PATH`);
          verifiedAwsPath = awsPath;
          awsVerificationTime = Date.now();
          return awsPath;
        }
      ...
      console.log(`[Main] AWS CLI PATH detection failed: ${error.message}`);
    }

    // Fallback 2: Shell environment with full profile loading
    console.log("[Main] Fallback 2: AWS CLI shell environment detection");
    try {
      const { stdout } = await execAsync(
        "bash -l -c 'which aws 2>/dev/null || echo NOTFOUND'"
      );
      const awsPath = stdout.trim();
      if (awsPath && awsPath !== "NOTFOUND") {
        console.log(`[Main] Found AWS CLI via shell: ${awsPath}`);
        const { stdout: versionOutput } = await execAsync(
          `"${awsPath}" --version`
        );
        if (
          ...
          console.log(`[Main] ✅ SUCCESS: AWS CLI verified via shell`);
          verifiedAwsPath = awsPath;
          awsVerificationTime = Date.now();
          return awsPath;
        }
      ...
      console.log(`[Main] AWS CLI shell detection failed: ${error.message}`);
    }

    // If we get here, AWS CLI was not found anywhere
    console.error(`[Main] ===== AWS CLI NOT FOUND (${operation}) =====`);
    console.error("[Main] Searched paths:");
    ...
    throw new Error(
      "AWS CLI is not installed or not found. Please install AWS CLI using 'brew install awscli' or download from https://aws.amazon.com/cli/"
    );
  };

  // Helper function to expand glob patterns
  const expandGlobPath = (pattern) => {
    try {
      const fs = require("fs");
      const path = require("path");
      ...
      // Simple glob expansion for single * in path
        ...
        if (parts[i].includes("*")) {
          ...
          if (fs.existsSync(basePath)) {
            ...
            for (const item of items) {
              if (
                parts[i] === "*" ||
                item.match(new RegExp(parts[i].replace("*", ".*")))
              ) {
                expandedPaths.push(path.join(basePath, item, restPath));
              }
            }
          }
          break;
      ...
      return expandedPaths.length > 0 ? expandedPaths : [pattern];
    } catch (error) {
      return [pattern];
    }
  };

  // Legacy findKubectl function removed - replaced by detectKubectl

  // Legacy findAwsCli function removed - replaced by detectAwsCli

  // Add kubectl status check handler
  ipcMain.handle("aws-sso:check-kubectl-status", async () => {
    try {
      console.log("[Main] ===== KUBECTL STATUS CHECK =====");

      // Use bulletproof detection for status check
      const kubectlPath = await detectKubectl("status-check");
      console.log(`[Main] Status check using kubectl at: ${kubectlPath}`);

      // Check kubectl version
      const { stdout: versionOutput } = await execAsync(
        `${kubectlPath} version --client --output=json`
      );
      ...
      try {
        ...
        currentContext = contextOutput.trim();

        // Get cluster info if context exists
        const { stdout: clusterOutput } = await execAsync(
          `${kubectlPath} cluster-info --request-timeout=5s`
        );
        clusterInfo = clusterOutput;
      } catch (contextError) {
        // Context might not be set or cluster might not be accessible
        console.log("[Main] No kubectl context set or cluster not accessible");
      }

      console.log("[Main] kubectl status check successful");
      console.log(
        `[Main] Verified kubectl path stored: ${verifiedKubectlPath}`
      );
      return {
        available: true,
        ...
        message: "kubectl is available and ready",
        kubectlPath: kubectlPath, // Store for future use
        detectionTimestamp: Date.now(),
      };
    } catch (error) {
      console.error("[Main] kubectl status check failed:", error.message);

      let errorMessage = "kubectl is not available";
      if (
        error.message.includes("not found using any detection strategy") ||
        error.message.includes("command not found") ||
        error.message.includes("not recognized") ||
        error.message.includes("not found in common paths")
      ) {
        errorMessage =
          "kubectl is not installed or not found in common paths (/usr/local/bin, /opt/homebrew/bin, etc.)";
      }
  ...
  // Add cache clearing handler for debugging
  ipcMain.handle("aws-sso:clear-tool-caches", async () => {
    console.log("[Main] Tool cache clear requested");
    clearToolCaches();
    return { success: true, message: "Tool caches cleared" };
  });
