...
contextBridge.exposeInMainWorld("awsSso", {
  ...
  pollToken: (deviceCode) => {
    console.log(
      `[Preload] AWS SSO: Polling token for device code ${deviceCode}`
    );
  ...
  listAccounts: (accessToken) =>
    ipcRenderer.invoke("aws-sso:list-accounts", { accessToken }),

  // List roles for an account
  listAccountRoles: (accessToken, accountId) =>
    ipcRenderer.invoke("aws-sso:list-account-roles", {
      accessToken,
      accountId,
    }),

  // Get credentials for a specific role
  getRoleCredentials: (accessToken, accountId, roleName) =>
    ipcRenderer.invoke("aws-sso:get-role-credentials", {
      accessToken,
      accountId,
      roleName,
    }),
  ...
  // Set default profile
  setDefaultProfile: (accountId, roleName) =>
    ipcRenderer.invoke("aws-sso:set-default-profile", { accountId, roleName }),

  // Run terminal command
  runTerminalCommand: (options) =>
    ipcRenderer.invoke("aws-sso:run-terminal-command", options),

  // Open a system terminal window with zsh
  openTerminal: (options) =>
    ipcRenderer.invoke("aws-sso:open-terminal", options),

  // Login to ECR
  loginToEcr: (accountId, roleName) =>
    ipcRenderer.invoke("aws-sso:login-ecr", { accountId, roleName }),

  // Login to CodeArtifact
  loginToCodeArtifact: (accountId, roleName) =>
    ipcRenderer.invoke("aws-sso:login-codeartifact", { accountId, roleName }),

  // Check Docker status
  checkDockerStatus: () => ipcRenderer.invoke("aws-sso:check-docker-status"),

  // List EKS clusters
  listEksClusters: (accessToken, accountId, roleName, region) =>
    ipcRenderer.invoke("aws-sso:list-eks-clusters", {
      accessToken,
      accountId,
      roleName,
      region,
  ...
  // Set kubectl context
  setKubectlContext: (accessToken, accountId, roleName, clusterName, region) =>
    ipcRenderer.invoke("aws-sso:set-kubectl-context", {
      accessToken,
      accountId,
      roleName,
      clusterName,
  ...
  checkKubectlStatus: () => ipcRenderer.invoke("aws-sso:check-kubectl-status"),
