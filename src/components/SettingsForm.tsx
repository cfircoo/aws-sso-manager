...
export function SettingsForm({ onClose, onLogout, isAuthenticated = false }: SettingsFormProps) {
  ...
  const [currentKubectlContext, setCurrentKubectlContext] = useState<string | null>(null);
  const [isLoadingKubectlContext, setIsLoadingKubectlContext] = useState(false);
  ...
  useEffect(() => {
    // Load current kubectl context on mount
    loadCurrentKubectlContext();
  }, []);

  const loadCurrentKubectlContext = async () => {
    if (!electron?.getCurrentKubectlContext) return;
    
    setIsLoadingKubectlContext(true);
    try {
      const context = await electron.getCurrentKubectlContext();
      setCurrentKubectlContext(context);
    } catch (error) {
      console.error('Error loading kubectl context:', error);
      setCurrentKubectlContext(null);
    } finally {
      setIsLoadingKubectlContext(false);
    }
  };
  ...
  return (
    <div className="p-8 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {/* Kubectl Context Section */}
        <div className="glass-card p-6 animate-slide-in" style={{ animationDelay: '250ms' }}>
          <div className="flex items-center space-x-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
              <Box className="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 className="text-xl font-semibold text-primary">Kubernetes Context</h2>
              <p className="text-sm text-tertiary">Current kubectl context information</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-4 bg-bg-surface rounded-xl border border-glass-border">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <div className="font-semibold text-primary">Current Context</div>
                <div className="text-sm text-tertiary font-mono">
                  {isLoadingKubectlContext ? (
                    ...
                    <span className="text-tertiary">No context set</span>
                  )}
                </div>
              </div>
            </div>
            <button 
              type="button"
              onClick={loadCurrentKubectlContext}
              disabled={isLoadingKubectlContext}
              className="btn-secondary hover:scale-105 transition-all duration-200"
              title="Refresh kubectl context"
            >
              <Box size={16} />
              <span>Refresh</span>
            </button>
